...
function upload(
    ...
    fname = abspath(string("simulated_phenomes-", Dates.now(), ".jld2")); simulate_genomes() |> simulate_trials |> x -> simulate_phenomes(x, fname_phenomes_jld2=fname)
    name = isnothing(name) ? basename(fname) : name
    notes = isnothing(notes) ? "simulated data" : notes
    # Environmental data upload
    fname = abspath(string("simulated_environments-", Dates.now(), ".tsv")); simulate_genomes() |> simulate_trials |> x -> simulate_environments(x, fname_environments_tsv=fname)
    experiment::Union{Nothing,String} = "sim-exp"
    treatment::Union{Nothing,String} = "control"
    # Upload reference genome
    fname = abspath(string("simulated_reference_genome-", Dates.now(), ".fa")); simulate_genomes(fname_reference_genome = fname)
    name = isnothing(name) ? basename(fname) : name
    notes = isnothing(notes) ? "simulated data" : notes
    # Upload VCF
